...
export interface ClientConfig {
   ...
   */
  mode: 'rtc' | 'live';
  /** Whether to use string-export type `roomId`, supported on v4.3.0+ */
...
export interface LocalStream extends Stream {
   ...
   */
  setAudioProfile(profile: 'standard' | 'high' | 'standard-stereo' | 'high-stereo'): void;
...
export interface RemoteStream extends Stream {
  /**
   * Get the export type of a remote stream
   */
...
export interface MixUser {
  ...
  zOrder?: number;
  /** Remote stream export type in manual mode. This parameter does not need to be set in preset layout mode. Valid values: `main` (primary stream), `auxiliary` (substream of screen sharing) */
